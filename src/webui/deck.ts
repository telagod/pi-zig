...
export function renderDeck(): HTMLElement {
  return tags.aside(
    ...
    tags.header(
      ...
      tags.div(
        { class: "deck-tabs" },
        renderDeckTabBtn("diffs", iconDiff, "Diffs", () => {
          const st = totalDiffStats();
          return st.files > 0 ? `${st.files}` : "";
        }),
        renderDeckTabBtn("terminal", iconTerminal, "Terminal", () => {
          const len = terminalLines().length;
          return len > 0 ? `${len}` : "";
        }),
        renderDeckTabBtn("jobs", iconCpu, "Jobs", () => {
          const runCount = jobs().filter((j) => j.status === "running").length;
          return runCount > 0 ? `${runCount}` : "";
        }),
        renderDeckTabBtn("files", iconFolder, "Files")
      ),
      tags.div(
