//! ai_tests.zig —— ai.zig 的单测主体(序列化/流式解析/缓存断点/思考级别)。
//! 拆自 ai.zig:测试体量超实现两倍,主文件只留代码。ai.zig 尾部以
//! `test { _ = @import("ai_tests.zig"); }` 引回,测试收集不变。
...
const serializeResponses = ai.serializeResponses;
const serializeOpenAI = ai.serializeOpenAI;
const serializeOpenAIThink = ai.serializeOpenAIThink;
const serializeAnthropic = ai.serializeAnthropic;
const serializeAnthropicThink = ai.serializeAnthropicThink;
const clampProviderImages = ai.clampProviderImages;
...
const ToolDef = ai.ToolDef;
const Usage = ai.Usage;
const RunResult = ai.RunResult;
const Callbacks = ai.Callbacks;
const ThinkLevel = ai.ThinkLevel;
const ToolAcc = ai.ToolAcc;
