...
默认开新会话（不带旧上下文开工）；`piz -c` 续载该目录最近一次会话并把历史画进对话区，开而未言即退的空会话不作续载目标。输入和状态钉在屏幕底，对话在上面软换行。对话区可以**拖选复制**（原生终端选区；没有开鼠标按键跟踪）。`PageUp` / `PageDown`、滚轮、`Ctrl+↑/↓` 翻历史。不用表情符号。`/copy` `/dump` 仍是剪贴板后备。

裸 `piz` 开场画一张欢迎卡（omp 式）：顶边版本号，左栏 logo + Welcome back! + 当前模型/provider，右栏 Tips（`/` 命令、`?` 快捷键、`!` 直跑 bash、`@` 嵌文件）+ 最近会话（标题 + 相对时间·轮数），卡下挂一条轮换 Tip；卡随对话上滚留存。`piz -c` 续载时不画欢迎卡、历史直接回放。输入框顶边内嵌信息条：模型 · think 档 │ 目录 ↳ 分支 │ 成本 · ctx 占用；页脚收成一行（目录·分支·会话 + `? for shortcuts`）。窄终端降级：&lt;60 列顶边只留模型 + ctx%，&lt;40 列顶边纯线、页脚回两行。完整栏（授权、上下文文件、用量）仍在 `/status`，按需才画进对话。输入是底栏框，不是壳上的 `>`。空框按 `?` 看快捷键；`r` 重发上一轮。输入 `@./` 弹出工作区文件补全（Tab / Enter 填入，↑↓ 选择；默认不列 gitignored / 跳过目录）。

对话是分块分层，靠左栏、缩进和字重区分，不是同一列淡灰。用户每行一条亮竖线 `▎`（加粗正文，没有 `┌/└` 空行）；思考退后两格、淡斜体（`Ctrl+T` 收成 `· thought` 一行）；助手正文同样缩进两格、正常色、没有竖线；工具默认折叠成一行 `▸` 名（加粗）+ 预览（淡）+ 状态，`Ctrl+O` 展开为 omp 式盒子：顶边 `╭─ 名 · 预览 ─ 状态 ─╮`、正文 `│ … │`（超 20 行截断）、底边 `╰─ Wall 耗时 · Nln ─╯`；`!cmd` 的结果也走同一盒子。块与块之间空一行；同轮兄弟工具中间不加空行。忙碌时输入框上方一行 `Working · 12s · esc to interrupt`（pi 式，比对话更淡），有子代理并行时再加 `● 名字 · 工具 · 12s` 摘要行（pi-subagents 插件式，至多两行）。
...
▎ your message
  ...
  assistant text
    ...
    ▸ bash  zig test src/foo.zig  ok 1.2s 12ln          ← 折叠态(Ctrl+O 展开成盒)
⠹ Working (12s • esc to interrupt)
╭── deepseek/v4-flash · max ── ~/project/pi-zig ↳ main* ── ctx 9% (1.2k/128k) ──╮
│ › _                                                                           │
╰───────────────────────────────────────────────────────────────────────────────╯
 ~/project/pi-zig (main*) · 1786741809022                        ? for shortcuts
```

信息条（输入框顶边）与页脚同源，随 `/model`、`Alt+,/.`（或 `/think`）以及每轮结束的 usage 回调刷新。占用是 `est_ctx / 窗口`（`1.2k/128k 9%`），不是单独一个 `0%`。宽度不够时顶边按 成本 → 分支 → think 档 → ctx 绝对数 的顺序弃段，模型名最后才截；&lt;40 列顶边退化为纯线，信息回页脚两行（pi 旧形）。缓存命中率等完整指标在 `/status` 卡。思考档是当前选的（默认 `high`）。
...
| `!command` | 执行 shell 命令，输出发给模型（**后台线程跑，不冻结 TUI**：执行中转圈、可继续打字，`Ctrl+C` 可取消；完成后输出进对话流自动接续模型） |
| `!!command` | 执行 shell 命令，输出**不**发给模型（自己看；同样后台跑） |
...
| `/compact` | 立即压缩上下文（密图 + 摘录，不调模型）；成功后给一行通知 `context compacted · kept N tok · N msgs folded` |
