...
对话是分块分层，靠左栏、缩进和字重区分，不是同一列淡灰。用户每行一条亮竖线 `▎`（加粗正文，没有 `┌/└` 空行）；思考退后两格、淡斜体（`Ctrl+T` 收成 `· thought` 一行）；助手正文同样缩进两格、正常色、没有竖线；工具再缩两格，一行写完 `▸` 名（加粗）+ 预览（淡）+ 状态，默认折叠，`Ctrl+O` 才在下面展开正文。块与块之间空一行；同轮兄弟工具中间不加空行。忙碌时输入框上方一行 `Working · 12s · esc to interrupt`（pi 式，比对话更淡），有子代理并行时再加 `● 名字 · 工具 · 12s` 摘要行（pi-subagents 插件式，至多两行）。
