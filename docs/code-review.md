...
| P2 | provider 怪癖隔离到适配层;大文件按边界拆 | 每加 provider 都要动主循环,review 成本 | 标记清单在 `ai_markers.zig`;tui/tools 已拆;main 已拆 `app_pickers`/`app_views`/`app_worker`(2264→1455 行,剩 App 壳+runInteractive+CLI 派发);ai 仍大 |
